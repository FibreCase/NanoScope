...
void taskBluetoothRecv(void* arg)
{
    ...
    while (1) {
        ...
        if (len > 0) {
            int readLen = bluetoothSerial.readBytes(buf, sizeof(buf));
...
void bluetoothFsm(uint8_t buf)
{
    ...
    case BT_FSM_ID:
        if (buf == BT_RECV_ID_X_RANGE || buf == BT_RECV_ID_Y_RANGE || buf == BT_RECV_ID_X_OFFSET || buf == BT_RECV_ID_Y_OFFSET || buf == BT_RECV_ID_VOLTAGE || buf == BT_RECV_ID_SAMPLE_RATE || buf == BT_RECV_ID_TRIGGER || buf == BT_RECV_ID_TRIGGER_VOLTAGE) {
            bt_recv_id = buf;
            bt_recv_count = 0;
